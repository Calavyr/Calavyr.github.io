...
import { PixelBehaviour, GravityBehaviour, FluidBehaviour, PlantBehaviour, getBehaviour, FlammableBehaviour, BatteryPartBehaviour, WireBehaviour, LampBehaviour, SteamGeneratorBehaviour } from './behaviours.js'
...
export class Pixel {
    ...
    constructor(id: number) {
        ...
        this.fixedThermalEnergy = info.fixedThermalEnergy
    ...
    clone(): Pixel {
        ...
        p.fixedThermalEnergy = this.fixedThermalEnergy
        return p
    }
    ...
    log(): void {
        ...
        console.group(
            `%cPixel #${this.info.name} (${this.id})%c | Temp: %c${this.temperature.toFixed(1)}°C%c | Behaviours: [%c${behaviourSummary}%c]`,
            'color: #00ffff; font-weight: bold;', // Cyan ID
            'color: #aaa;', 
            'color: #ff9900; font-weight: bold;', // Orange Temp
            'color: #aaa;',
            'color: #ff00ff;',                     // Magenta Behaviours
            'color: #aaa;'
        );
...
export const interactions: Record<number, Record<number, Interaction>> = {
    ...
    5: {
        ...
        16: (smoke, smokePos, filter, filterPos, current, next, grid) => {
            next[smokePos.y][smokePos.x] = new Pixel(0)
            grid.updated[smokePos.y][smokePos.x] = true
        }
    }
